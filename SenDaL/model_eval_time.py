...
from Models import check_inference_time
...
for nWD, windowDataset in zip(range(len(windowDatasets)), windowDatasets):
    ...
    for i, testDataLoader in zip(range(CROSS_VALID), testDataLoaders):
        ...
        if TYPE == 'single':
            time = check_inference_time(uModel, 0, torch.Tensor(testX).to(device), testy, device)
        else:
            time = check_inference_time(uModel, -1, torch.Tensor(testX).to(device), testy, device)
...
print('Home1: ', np.sqrt(np.mean(time_list[:40])))
print('Home2: ', np.sqrt(np.mean(time_list[40:80])))
print('Home3: ', np.sqrt(np.mean(time_list[80:])))
print('Overall: ', np.sqrt(np.mean(time_list)))
